...
- ★★**옷을 추출하면 그 옷 말고는 아무것도 딸려오면 안 된다** (대표 2026-08-27:
  "앞으로 옷 추출할 때 그 레이어 제외하고는 얼굴이나 하의나 캐릭터 다른 곳 절대 추출하지 마라").
  브라탑을 뽑을 때 상의 슬롯 영역이 y67~188 이라 **하의 허리까지 딸려 나왔고**,
  프레임마다 옷 경계가 23px 씩 튀어 대표가 12프레임을 손으로 다 지우느라 **하루를 썼다.**
  → 추출 직후 반드시 **두 개를 돌린다**:
  `python tools/drop_stray.py char/items/<id>.png`  ← 본체와 떨어진 조각을 버린다
  `python tools/item-purity-check.py char/items/<id>.png --base char/walk_female.png --slot top`
  ★**슬롯 영역만으로는 부족하다.** 상의 영역이 y67~188 이라 그 안의 팔 그림자·목·머리카락 끝·
    하의 허리가 같이 잡힌다(대표 8-27: "왜 상의 추출이면 상의 근처만 추출해야 하는데
    계속 다른 애들도 추출해버리는 거임?"). 크기 임계(min_frag)로는 150px 짜리 큰 조각을 못 거른다.
    옷은 프레임마다 **한 덩어리**다 — 본체에서 떨어져 나온 것은 잔여물이다(망고나시 2,194px).
  살색·하의를 베껴 왔으면 FAIL 이다. 프레임 흔들림은 경고로 나온다(걸음 자세일 수 있다).
