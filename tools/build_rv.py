...
OUT = REPO + "/rv"
E = html.escape
AV = "11"   # 오디오 캐시버스터 — 음원을 다시 뽑으면 올린다
...
CANDS = [
    # (NPC, 상황, 대사, char, kind, 안내문, 대표가 고른 것)
    ("민준이", "처음 만났을 때", "형아! 누나! 나랑 놀자! 오늘의 머피들 봤어? 매칭 탭에서 새 친구 구경하고 와!", "kid", "hi",
     "", "C"),
    ("관리인 박씨", "퀘스트 완료", "오, 도장 찍고 왔구먼! 부지런한 게 최고야.", "keeper", "done", "", "현재 것 유지(대표 9-06)"),
    ("강 코치", "퀘스트 완료", "그래, 그 기세야! 꾸준한 놈이 이긴다.", "trainer", "done",
     "대표님 지적: '너무 찐따 같음'. 코치 설정에 '가슴에서 울리는 힘 있고 자신감 있는 목소리, 얇고 소심한 톤 금지'를 박고 4개 뽑았습니다(같은 Puck 목소리).", ""),
    ("민준이", "보상까지 받은 뒤", "내일 또 놀러 와! 약속!", "kid", "idle", "", "B"),
]
cands_html = []
for who, what, txt, ck, kind, note, picked in CANDS:
    slot = "후보: " + who + " " + what
    if picked:
        cands_html.append(
            '<section class="npc"><header class="nhd"><h3>%s</h3><span class="age">%s</span>'
            '<span class="done">선택 완료 · %s &rarr; 반영됨</span></header>'
            '<p class="line">&ldquo;%s&rdquo;</p></section>' % (E(who), E(what), E(picked), E(txt)))
        continue
    ...
    if not files:
        note = (note + " " if note else "") + "(후보 뽑는 중 — 곧 올라옵니다)"
    cands_html.append(
