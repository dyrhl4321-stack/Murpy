...
- 슬롯 z순서(뒤→앞): `body → bottom → top → hair → hat`.
...
**Interfaces:**
- Produces:
  - `window._CHAR_LAYER_ORDER: string[]` = `['body','bottom','top','hair','hat']`
  - `window._charBuildLayers(): void` — `#charworld-avatar` 안에 `.cw-layer[data-slot]` 겹 생성(멱등), 컨테이너 자체 배경 제거.
  - `window._charEquippedSheets(cfg): {body,bottom,top,hair,hat}` — 각 값은 시트 URL 또는 null.
...
```javascript
// ===== 다층 의상 레이어 =====
window._CHAR_LAYER_ORDER = ['body', 'bottom', 'top', 'hair', 'hat']; // 뒤→앞 z순서
...
window._charEquippedSheets = function (cfg) {
  ...
  const out = { body: b.src, bottom: null, top: null, hair: null, hat: null };
  ['bottom', 'top', 'hair', 'hat'].forEach(function (slot) {
...
- Create: `char/items/bottom_ph.png`, `char/items/top_ph.png`, `char/items/hair_ph.png`, `char/items/hat_ph.png` (생성물)
...
- Produces: `char/items/<slot>_ph.png` 4장(각 walk.png와 동일 픽셀 규격), `CHAR_ITEMS` 신규 4항목(각 `sheet` 필드).
...
```
Expected 출력: `saved hat_ph ...` 등 4줄, 크기 = walk.png와 동일. `char/items/`에 png 4개 존재.
...
const ITEMS = ['items/bottom_ph.png','items/top_ph.png','items/hair_ph.png','items/hat_ph.png'];
const SHEETS = ['body'].concat(['bottom','top','hair','hat']);
const URLS = { body: BODY, bottom: ITEMS[0], top: ITEMS[1], hair: ITEMS[2], hat: ITEMS[3] };
...
- 4장 임시 밴드(모자=파랑/머리=검정/상의=빨강/하의=초록)가 **몸통과 함께** 이동하고 프레임이 바뀐다.
...
### Task 3: 꾸미기 UI 5슬롯 확장 + 실시간 반영

꾸미기 패널에 하의·모자 탭을 추가하고, 아이템 선택/해제 시 **걷는 캐릭터에 즉시** 반영되게 배선한다.
...
```javascript
window.CHAR_DEFAULT = { skin: '#F0C9A0', hair: 'hair_short', hairColor: '#2B2B2B', top: 'top_tee', topColor: '#3D7EFF', bottom: null, hat: null, mask: null, body: 'human' };
```
...
```javascript
  if (['hair', 'top', 'bottom', 'hat', 'mask'].includes(slot)) {
...
- 탭에 헤어/상의/하의/모자/색상/가면 6개 보임.
- 하의(샘플)·모자(샘플) 등 각 슬롯 아이템 선택 시 **방에서 걷는 캐릭터에 해당 색 밴드가 즉시** 붙는다.
- 여러 슬롯 동시 착용(예: 모자+상의+하의) → 겹이 z순서대로(모자 최상단) 함께 걷는다.
...
- **Spec 커버리지:** 4.1 슬롯/z순서→Task1 `_CHAR_LAYER_ORDER`. 4.2 HTML 컨테이너화→Task1 `_charBuildLayers`. 4.3 렌더 함수→Task1. 4.4 데이터 모델(bottom/hat)→Task3 Step2. 4.5 프리뷰 통일→Task4. 4.6 꾸미기 배선→Task3. 5장 아트 파이프라인(임시 시트)→Task2. 6장 검증→각 Task 검증 스텝 + Task2 하니스. 누락 없음.
