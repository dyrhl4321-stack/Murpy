...
FRAMES = 6
SRC_CELL = 96
# ★★자르는 범위 (2026-08-18 두 번째 수정)
#   처음엔 고리만(33,33,64,61 = 31x28) 잘랐다가 **애니메이션을 통째로 잘라 버렸다.**
#   이 에셋은 고리가 도는 게 아니다 — 고리는 제자리에 있고 거기서 **불꽃이 위로 솟았다
#   사라진다.** 그 불꽃이 프레임 2~4 에서 y=25 까지 올라오는데 y=33 에서 잘라내니
#   원만 남았고, 남은 건 자잘한 점 변화뿐이라 "공항 무빙워크처럼 옆으로 흘러가는"
#   느낌이 됐다(대표 지적).
BB = (31, 22, 66, 62)        # 솟는 불꽃까지 포함 (35x40)
RING_MID = 47                # 고리의 세로 중심(원본 좌표). 여기서 앞/뒤를 가른다
SX, SY = 4, 3                # 가로 4배 / 세로 3배. 2배면 솟는 불꽃까지 눌려 안 보인다(대표 8-18)
...
def build(sheet_path, name):
    ...
    w = (BB[2] - BB[0]) * SX
    h = (BB[3] - BB[1]) * SY
    # ★가르는 자리는 그림의 절반이 아니라 **고리의 세로 중심**이다.
    #   솟는 불꽃까지 넣으면 위쪽이 길어져서, 절반으로 자르면 고리를 가로질러 잘린다.
    half = (RING_MID - BB[1]) * SY
