    ...
    for f in range(frames):
        ...
        for (x, y) in safe:
            ...
            if 0 <= nx < W and ((nx, y) in mask or px[nx, y][3] <= 128):
                fpx[nx, y] = px[x, y]
        # 밝기 맥동. 좌우 흔들림은 위쪽만 걸린다(밑동은 장작과 붙어 못 흔든다) — 그 상하 이질감을
        # 밝기로 보정한다: '하단일수록 진폭을 키워' 못 흔드는 밑동이 오히려 더 확실히 깜박이게 하고,
        # 세로 위상차로 아래→위로 흐르는 상승 물결을 만들어 불꽃 전체가 한 덩어리로 살아있게 한다.
        # (예전 균일 14%는 상단 좌우흔들림에 묻혀 '아래는 안 깜빡'으로 보였다 — 대표 반복 지적.)
        for (x, y) in mask:
            p = fpx[x, y]
            if p[3] > 128 and is_flame(p):
                yr = y / float(FLAME_MAX_Y)                 # 0=꼭대기, 1=밑동
                amp = 0.16 + 0.24 * yr                      # 상단 0.16 → 밑동 0.40
                g = 1.0 + amp * math.cos(phase - y * 0.28)  # 세로 위상차 = 상승 물결
