...
★RUNNING 을 나중에 추가했다(2026-08-25) — '따라와' 를 켜면 강아지가 **달려온다**.
  처음엔 자유 행동에 뛰기가 없어서 뺐는데, 따라오기가 생기면서 필요해졌다.
...
ANIMS = ['walk', 'stand', 'sit', 'eat', 'act1', 'act2', 'sleep', 'lay', 'run']
ROW_MAP = {                   # 동작 -> 출력 시작 행
    'walk': 0, 'stand': 4, 'sit': 8, 'eat': 12,
    'act1': 16, 'act2': 20,   # 고양이 = 핥기/관찰, 강아지 = 냄새맡기/장난치기
    'sleep': 24,              # 2행만 쓴다(좌/우)
    'lay': 26,
    'run': 30,                # 뛰기 — '따라와' 를 켰을 때 강아지가 달려온다(대표 8-25)
}
ROWS = 34
...
SRC = {
    ...
    'dog': {
        ...
        'sleep': ('SLEEPING',  [0, 1]),
        'lay':   ('LAYING',    [1, 3, 5, 7]),
        # ★강아지 RUNNING 은 WALKING 과 같은 구성이다 — 1·3·6 은 짖으며 뛰기 교체용
        'run':   ('RUNNING',   [0, 2, 4, 5]),
    },
}
