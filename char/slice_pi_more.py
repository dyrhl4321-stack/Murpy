...
ITEMS = [
    ...
    # ── 책상의 **측면** (신규 아이템이 아니다) ──────────────────
    #    ★처음에 '하이 테이블'이라는 새 가구로 등록했다가 대표 지적으로 바로잡았다:
    #      "너가 넣은 하이테이블이 우리 책상 왼쪽이나 오른쪽으로 돌린 거임".
    #      결정적 증거 = **나뭇결 방향이 90도 돌아가 있다**(정면은 가로결, 측면은 세로결).
    #      다리 위치와 상판 밑 회색 그늘도 정면(232,130~280,158)과 같은 계열이다.
    #    → 새 가구를 늘리는 게 아니라 **기존 책상에 돌리기를 붙이는** 것이 맞다.
    #    ★두 번째 교정(대표 8-16): 이 둘은 **같은 가구의 좌/우가 아니다.**
    #      "돌려봤는데 작아졌다 커졌다 한다. 작아진 버전이 우리 기본 테이블의 돌린 버전임".
    #      크기가 튄 것이 곧 증거였다 — 23x47 은 책상(48x28)의 측면,
    #      23x42 는 테이블(36x28)의 측면이다. 폭(=깊이)은 23 으로 같고 길이만 다르다.
    #      → 각자 자기 가구에 붙인다. 한 가구에 2방향(정면·측면)씩이다.
    ('pi_desk_d1',  '책상(측면)',   164, 106, 187, 153),
    ('pi_table_d1', '테이블(측면)', 197, 111, 220, 153),

    # ── 신규: 조리대 2종 × 2방향 ────────────────────────────────
#    ★'주방 카운터'라고 붙였다가 대표 지적으로 고쳤다 — **싱크가 없다.**
#      시트에는 싱크 달린 것이 따로 있다(163,224~221,254). 그건 '싱크대'다.
#      상판+문짝 = 조리대 / 상판+서랍 = 주방 서랍장.
    ('pi_counter',        '대리석 수납장',       171, 195, 212, 222),
    ('pi_counter_d1',     '대리석 수납장(뒤)',   171, 259, 212, 286),
    ('pi_counter_s',      '대리석 서랍장',       229, 195, 251, 222),
    ('pi_counter_s_d1',   '대리석 서랍장(뒤)',   229, 259, 251, 286),
]
